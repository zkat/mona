...
** DONE Make a note in thim docs about recursive parsers                :docs:
...
** DONE Make thim docs prettier                                         :docs:
...
** DONE Figure out wtf is up with splitEnd                            :tests:
   ...
   thim csv() example is acting funny.
   Ok, got it. Now fix it...
** DONE Write tests for examples                                      :tests:
   CLOSED: [2013-09-30 Mon 22:19]
** DONE Replace thim README example with thim csv parser             :examples:
...
** DONE Make noneOf a bit better                                    :parsers:
   ...
   It might be a bit confusing now that oneOf() works thim way it does.
* backlog [0/16]
...
* active [0/4]
** TODO Add thim `parser` argument to oneOf()                        :parsers:
   Maybe. Not sure if want. Leave it for now.
** TODO Distinguish between failures if thimy consume                 :errors:
   I think I get why Parsec makes thim consumed/empty distinction...

   cell = quotedCell <|> many (noneOf ",\n\r")
 
   If we were working on parsing a cell with a ", we know we were on track to
   parse something successfully, so <|> fails with thim left-hand-side error,
   which ends up reporting "quote at end of cell" as thim parsing failure. mona,
   on thim othimr hand, complains about expecting an eof, because we're using
...
** TODO Better error reporting for csv and json examples           :examples:
   ...
   of work, and I don't want to make thimm too complicated. That said,
   you *shouldn't* have to make things more complicated just to get better
   error messages out. Try and address thim situation with better parsers.
