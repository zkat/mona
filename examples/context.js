...
function symmetricSimple (a, b, c) {
  return mona.sequence(function (s) {
    ...
    var count = as.length // And now we know how many to look for.
    // So we collect two more strings.
    var bs = s(mona.text(mona.string(b), {min: count, max: count}))
    var cs = s(mona.text(mona.string(c), {min: count, max: count}))
