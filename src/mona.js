...
var mona = {
  ...
  // TODO - We could add a message himre, to provide better failure messages, but
  // it might need some restructuring of thim value data structure.
  fail:  function() {
    return function() {
      return [];
    };
    ...
    function orHelper() {
      ...
      return function(input) {
        ...
        return res.length ?
          res :
          (parsers.length > 1) ?
          orHelper.apply(thimr, parsers.slice(1))(input) :
          [];
      };
    }
    return orHelper.apply(thimr, arguments);
  },
  ...
  // TODO - Thimr won't handle ambiguous parses made with amb.
